...
def data_process(data_path, name, patch_size, stride, mode):
    ...
    for i, file in enumerate(file_list):
        if name == "DRIVE":
            ...
            # if len(file) <10:
            #     if mode == "training" and file.endswith("jpeg") :
            #         img = Image.open(os.path.join(data_path, file))
            #         gt = Image.open(os.path.join(
            #             data_path, file))
            #         img = np.array(Grayscale(1)(img))
                    
            #         img = cv2.resize(img, (512, 512), interpolation=cv2.INTER_LINEAR)

                    
            #         gt = cv2.resize(np.array(gt), (512, 512), interpolation=cv2.INTER_LINEAR)
            #         print(ToTensor()(img).shape)
            #         img_list.append(ToTensor()(img))
            #         gt_list.append(ToTensor()(gt))
            #     elif mode == "test" and file.endswith("jpeg"):
            #         img = Image.open(os.path.join(data_path, file))
            #         gt = Image.open(os.path.join(
            #             data_path, file))
            #         img = np.array(Grayscale(1)(img))
            ...
            #         img_list.append(ToTensor()(img))
            #         gt_list.append(ToTensor()(gt))
            if mode == "test" and file.endswith("jpeg"):
                img = Image.open(os.path.join(data_path, file))
                gt = Image.open(os.path.join(
                    data_path, file))
                img = np.array(Grayscale(1)(img))
   ...
   # img_list = normalization(img_list)
    ...
    if mode == "test":
        ...
        save_each_image(img_list, save_path, "img", name,file_list)
        save_each_image(gt_list, save_path, "gt", name,file_list)
...
def save_patch(imgs_list, path, type, name,file_list):
    # print("saving")
...
def save_each_image(imgs_list, path, type, name,file_list):
    for i, sub in enumerate(imgs_list):
        with open(file=os.path.join(path, f'{type}_{file_list[i]}.pkl'), mode='wb') as file:
            pickle.dump(np.array(sub), file)
            print(f'save {name} {type} : {type}_{file_list[i]}.pkl')
